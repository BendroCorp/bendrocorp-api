...
class DormantApprovalWorker
  ...
  def perform(*args)
    ...
    dormant_approvals = ApprovalApprover.where('approval_type_id < 4 AND created_at <= ? AND last_notified is not null AND last_notified <= ? AND required = ?', Time.now - 2.days, Time.now - 12.hours, true)
