class SystemMapLocationsController < ApplicationController
  ...
  def index
    ...
    render json: @system_map_locations.to_json(include: { system_map_images: { methods: [:image_url_thumbnail, :image_url], include: { created_by: { methods: [:main_character] } } }, mission_givers: {}, faction_affiliation: {} }, methods: [:kind, :primary_image_url, :parent, :title_with_kind])
  end

  # GET /system_map_locations/1
  def show
    if @system_map_location
      render json: @system_map_location.to_json(include: { system_map_images: { methods: [:image_url_thumbnail, :image_url], include: { created_by: { methods: [:main_character] } } }, mission_givers: {}, faction_affiliation: {} }, methods: [:kind, :primary_image_url, :parent, :title_with_kind])
    else
  ...
  def create
    ...
    if @system_map_location.save
      render json: @system_map_location.to_json(include: { system_map_images: { methods: [:image_url_thumbnail, :image_url], include: { created_by: { methods: [:main_character] } } }, mission_givers: {}, faction_affiliation: {} }, methods: [:kind, :primary_image_url, :parent, :title_with_kind]), status: :created
    else
