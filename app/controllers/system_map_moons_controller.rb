class SystemMapMoonsController < ApplicationController
  ...
  def index
    ...
    render json: @system_map_moons.to_json(include: { system_map_images: { methods: [:image_url_thumbnail, :image_url], include: { created_by: { methods: [:main_character] } } }, settlements: {}, locations: { include: { mission_givers: {} }}, system_objects: {}, faction_affiliation: {}, jurisdiction: {} }, methods: [:kind, :primary_image_url, :parent, :title_with_kind])
  end

  # GET /system_map_moons/1
  def show
    if @system_map_moon
      render json: @system_map_moon.to_json(include: { system_map_images: { methods: [:image_url_thumbnail, :image_url], include: { created_by: { methods: [:main_character] } } }, settlements: {}, locations: { include: { mission_givers: {} }}, system_objects: {}, faction_affiliation: {}, jurisdiction: {} }, methods: [:kind, :primary_image_url, :parent, :title_with_kind])
    else
  ...
  def create
    ...
    if @system_map_moon.save
      render json: @system_map_moon.to_json(include: { system_map_images: { methods: [:image_url_thumbnail, :image_url], include: { created_by: { methods: [:main_character] } } }, settlements: {}, locations: { include: { mission_givers: {} }}, system_objects: {}, faction_affiliation: {}, jurisdiction: {} }, methods: [:kind, :primary_image_url, :parent, :title_with_kind]), status: :created
    else
