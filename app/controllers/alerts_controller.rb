class AlertsController < ApplicationController
  ...
  def show
    if !@alert.nil?
      render status: 200, json: @alert.as_json(include: { star_object: { methods: [:kind, :primary_image_url] }, alert_type: {}, user: { only:[:id], methods: [:main_character] } })
    else
  ...
  def create
    ...
    if @alert.save
      render status: 201, json: @alert.as_json(include: { star_object: { methods: [:kind, :primary_image_url] }, alert_type: {}, user: { only:[:id], methods: [:main_character] } })
    else
