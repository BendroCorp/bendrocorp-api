class SystemMapMissionGiversController < ApplicationController
  ...
  def index
    ...
    render json: @system_map_mission_givers.to_json(include: { faction_affiliation: {}, system_map_images: { methods: [:image_url_thumbnail, :image_url], include: { created_by: { methods: [:main_character] } } } }, methods: [:kind, :primary_image_url, :primary_image_url_full, :parent, :title_with_kind])
  end

  # GET /system_map_mission_givers/:uuid
  def show
    if @system_map_mission_giver
      render json: @system_map_mission_giver.to_json(include: { faction_affiliation: {}, system_map_images: { methods: [:image_url_thumbnail, :image_url], include: { created_by: { methods: [:main_character] } } } }, methods: [:kind, :primary_image_url, :primary_image_url_full, :parent, :title_with_kind])
    else
  ...
  def create
    ...
    if @system_map_mission_giver.save
      render json: @system_map_mission_giver.to_json(include: { faction_affiliation: {}, system_map_images: { methods: [:image_url_thumbnail, :image_url], include: { created_by: { methods: [:main_character] } } } }, methods: [:kind, :primary_image_url, :primary_image_url_full, :parent, :title_with_kind]), status: :created
    else
